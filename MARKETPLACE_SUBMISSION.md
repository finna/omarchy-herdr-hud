...
Current release: v1.0.15. Chat extends to the top of the panel, with branding and agent count in the sidebar. H and Esc close the panel; polling is automatic. The selected card identifies the agent, and its tooltip shows available model details. Recognized conversations use formatted chat, with plain output preserved when formatting is unavailable.
